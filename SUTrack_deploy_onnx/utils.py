"""
SUTrack ONNX Runtime Deployment - Utility Functions
...
def preprocess(img_rgb: np.ndarray) -> np.ndarray:
    """
    Normalize an HxWxC uint8 RGB image to a (1, C*2, H, W) float32 array
    ready for ONNX (duplicated channels for 6-ch multi-modal input).
...
def sample_target(im: np.ndarray, target_bb: list, search_area_factor: float,
    ...
    Args:
        im:                 HxWx3 numpy image (any dtype)
