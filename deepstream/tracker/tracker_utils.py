...
def load_config(yaml_path: str) -> SimpleNamespace:
    """Load a YAML config file into a plain namespace."""
...
class TrackerConfig:
    """Tracker params extracted from a SUTrack experiment YAML config."""
...
def preprocess(img_rgb: np.ndarray) -> np.ndarray:
    """
    Normalize an H×W×3 uint8 RGB image to (1, 6, H, W) float32.

    Duplicates channels to produce the 6-ch multi-modal input expected by sutrack_t224.
    """
    img = img_rgb.astype(np.float32) / 255.0
    img = (img - _MEAN) / _STD
    img = img.transpose(2, 0, 1)                        # (3, H, W)
    img6 = np.concatenate([img, img], axis=0)           # (6, H, W)
...
def sample_target(im: np.ndarray, target_bb: list, search_area_factor: float,
                  output_sz: int) -> tuple:
    """
    Extract a square crop centered on target_bb, padded and resized to output_sz.
...
# ---------------------------------------------------------------------------
# Template annotation transform
# ---------------------------------------------------------------------------

def transform_image_to_crop(box_in: list, box_extract: list,
                             resize_factor: float, crop_sz: int) -> np.ndarray:
    """
    Transform bbox coordinates from original image space to normalized crop space [0,1].
    ...
    Returns:
        np.ndarray of shape (4,): [x1_n, y1_n, w_n, h_n] in [0,1]
    """
...
# ---------------------------------------------------------------------------
# IoU utility (used by TrackerManager)
# ---------------------------------------------------------------------------
